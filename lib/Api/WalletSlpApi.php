...
class WalletSlpApi
{
     ...
     * @throws \Mainnet\ApiException on non-2xx response
     * @throws \InvalidArgumentException
     * @return \Mainnet\Model\SlpTokenInfoResponseItem
     */
     ...
     * @throws \Mainnet\ApiException on non-2xx response
     * @throws \InvalidArgumentException
     * @return array of \Mainnet\Model\SlpTokenInfoResponseItem, HTTP status code, HTTP response headers (array of strings)
     */
    public function slpTokenInfoWithHttpInfo($slp_token_info_request)
    {
        ...
        try {
            ...
            switch($statusCode) {
                case 200:
                    if ('\Mainnet\Model\SlpTokenInfoResponseItem' === '\SplFileObject') {
                        $content = $responseBody; //stream goes to serializer
                    } else {
                        $content = (string) $responseBody;
                    }
                    ...
                    return [
                        ObjectSerializer::deserialize($content, '\Mainnet\Model\SlpTokenInfoResponseItem', []),
                        $response->getStatusCode(),
                        $response->getHeaders()
                    ];
            }

            $returnType = '\Mainnet\Model\SlpTokenInfoResponseItem';
            $responseBody = $response->getBody();
            if ($returnType === '\SplFileObject') {
                $content = $responseBody; //stream goes to serializer
            } else {
            ...
            switch ($e->getCode()) {
                case 200:
                    $data = ObjectSerializer::deserialize(
                        $e->getResponseBody(),
                        '\Mainnet\Model\SlpTokenInfoResponseItem',
                        $e->getResponseHeaders()
                    );
                    $e->setResponseObject($data);
                    break;
